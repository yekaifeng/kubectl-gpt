...
func RequestChatGptAPI(reqBody OpenAIRequest, apiEndpoint, apiKey string) (OpenAIResponse, error) {
	client := &http.Client{}
	...
	req, err := http.NewRequest("POST", apiEndpoint+"/v1/chat/completions", strings.NewReader(string(jsonReqBody)))
	if err != nil {
		return OpenAIResponse{}, err
	}
